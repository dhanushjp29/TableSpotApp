...
export default function BookingDetailPage() {
  ...
  return <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div className="mb-5 flex flex-wrap items-center justify-between gap-3"><h1 className="text-2xl font-bold text-text">Booking details</h1><div className="flex flex-wrap gap-2"><PdfDownloadButton size="sm" filename={bookingReceiptFilename} fetchData={async () => ({ booking })} serverReceipt={({ booking: data }) => ({ type: "booking", id: data._id })} renderDocument={({ booking: data }) => <BookingPdf booking={data} view="customer" />} />{booking.bookingCode && <Badge variant="primary">{booking.bookingCode}</Badge>}</div></div>
