...
function MobileNav({ isListingPage, isAuthenticated, dashboardPath, user }) {
  const [mobileOpen, setMobileOpen] = useState(false);

  const mobileLinkClasses = (isActive) =>
    `flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-medium transition-colors ${isActive
    ...
    <>
      <div className="flex items-center gap-2 md:hidden">
        ...
        <button
          ...
          className={`inline-flex h-10 w-10 items-center justify-center rounded-xl border transition-all ${isListingPage
          ...
          <nav className="mx-auto max-w-7xl space-y-1 px-4 py-4 sm:px-6">
            ...
            {isAuthenticated ? (
              ...
              <div className="flex flex-col gap-2 pt-1">
                <Link to={ROUTES.LOGIN} className="btn-outline w-full text-sm">
                  Login
                </Link>
                <Link to={ROUTES.REGISTER} className="btn-primary w-full text-sm">
                  Register
                </Link>
              </div>
            )}
...
function PublicLayout() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      <header
        className={`sticky top-0 z-40 border-b backdrop-blur-xl ${isListingPage
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  <span className="max-w-30 truncate text-sm font-medium text-text">
                    {user?.fullName || user?.name || "My Account"}
                  </span>
                </Link>
              </div>
      ...
      <main className="relative flex-1 overflow-hidden">
        <div className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-64 bg-linear-to-b from-primary/6 via-transparent to-transparent" />
        <Outlet />
      </main>
