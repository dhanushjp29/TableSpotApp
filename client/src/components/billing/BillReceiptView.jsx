    ...
    <div className="space-y-5">
      ...
      >
        ...
        <div className="p-5 sm:p-8 dark:bg-[#171b23]">
          ...
          {payments.length > 0 && <div className="mt-6 rounded-2xl border border-gray-300 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-[#11151b]"><p className="mb-3 text-[10px] font-bold uppercase tracking-widest text-gray-700 dark:text-slate-300">Payment history</p>{payments.map((payment, index) => <div key={`${payment.transactionId || index}`} className="flex justify-between gap-2 border-b border-gray-200 py-2 text-sm last:border-0 dark:border-white/10"><span className="min-w-0 truncate font-bold text-gray-900 dark:text-slate-100">{payment.paymentMethod}{payment.transactionId ? ` · ${payment.transactionId}` : ""}</span><span className="shrink-0 font-bold text-gray-950 dark:text-slate-100">{money(payment.amount)}</span></div>)}</div>}
        </div>
      </div>
