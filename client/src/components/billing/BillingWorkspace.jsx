...
export default function BillingWorkspace({ bill, billType, bookings = [], bills, initialTab = "editor", restaurants, tables, foods, loading, submitting, onBack, onNew, onNewOnline, onSelectBill, onRestaurantChange, onSave, onCreateOnline, onPrint, onDownloadPdf }) {
  ...
  return <div className="min-h-[calc(100vh-8rem)] rounded-3xl border border-red-100 bg-red-50/30 p-3 transition-colors dark:border-white/[0.07] dark:bg-[#0d1015] sm:p-5">
    ...
    <div className="grid gap-5 lg:grid-cols-[280px_minmax(0,1fr)]">
      ...
      <main className="rounded-2xl border border-red-100 bg-white p-4 shadow-sm transition-colors dark:border-white/[0.08] dark:bg-[#151920] dark:shadow-black/20 sm:p-6">
        ...
        {isOnlineCreate ? <OnlineBillStarter bookings={bookings} onCancel={onBack} onCreate={onCreateOnline} submitting={submitting} /> : tab === "receipt" && bill ? <BillReceiptView bill={bill} /> : <BillEditor bill={bill} billType={bill?.billType || "WALK_IN"} restaurants={restaurants} tables={tables} foods={foods} loading={loading} submitting={submitting} onRestaurantChange={onRestaurantChange} onSubmit={onSave} />}
      </main>
    </div>
  </div>;
}
